...
def gen_merkle_proof(leaves, pos):
    ...
    height = math.ceil(math.log(len(leaves), 2))  #log2^1000 = 9.96578
    # print(len(leaves), height)
    ...
    state += [b"\x00"] * padlen  #add 24 'b"\x00"' at the end of hashed list: 'state'
    ...
    for level in range(height):
        ...
        for i in range(2**(height - level - 1)):
            temp.append(hash_internal_node(state[2 * i], state[2 * i + 1]))  #hash two values to temp
        if level_pos % 2 == 0:  #append left or right side of hashed value to path
            path.append(state[level_pos + 1])
        else:
            path.append(state[level_pos - 1])
        state = temp  #update state
        level_pos >>= 1  #verifier.py line 54, to get the number of upper layer
